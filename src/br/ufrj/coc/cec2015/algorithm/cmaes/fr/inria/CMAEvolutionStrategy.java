      ...
      for (int iKoo = 0; iKoo < N; ++iKoo) {
        if (xmean[iKoo] == xmean[iKoo] + 0.2 * sigma * Math.sqrt(CoVar[iKoo][iKoo])) {
          appendMessage("NoEffectCoordinate: Mutation of size "
                  + 0.2 * sigma * Math.sqrt(CoVar[iKoo][iKoo])
  ...
  void testAndCorrectNumerics() { // not much left here
    ...
    if (getCountIter() > 1 || (getCountIter() == 1 && state >= 3)) {
      if (fit.fitness[0].val == fit.fitness[Math.min(sp.getLambda() - 1, sp.getLambda() / 2 + 1) - 1].val) {
        ...
        sigma *= fac;
        //warning("flat fitness landscape, consider reformulation of fitness, step-size increased by factor " + fac);
      }
    }
  ...
  public double[] init() {
    ...
    if (xmean == null) {
      ...
      if (typicalX != null) {
        ...
        error("no initial search point (solution) X or typical X specified"); // TODO Dirk: remove this error - set initial start point to mean value to avoid starting in a corner of the search space.
        xmean = new double[N];
...
//    }
   ...
   * @param x design variables of the population.
   */
  ...
  public double mahalanobisNorm(double[] x, double[] mean) {
    ...
    //       = (D^-1 Bmat' (x-mean))' * (D^-1 Bmat' (x-mean))
    	/* calculate z := D^(-1) * Bmat^(-1) * BDz into artmp, we could have stored z instead */
    for (i = 0; i < N; ++i) {
      for (j = 0, yi = 0.; j < N; ++j) {
...
//    			double sum = 0.0;
//    			for (j = 0, sum = 0.; j < N; ++j)
//    				sum += Bmat[j][i] * ((arxrepaired[fit.funValues[iNk].i][j] - arx[fit.funValues[iNk].i][j]));
...
//        }
    ...
    if (sp.getCcov() > 0 && iniphase == false) {
      ...
      for (i = 0; i < N; ++i) {
        for (j = (flgdiag ? i : 0);
                j <= i; ++j) {
          CoVar[i][j] = (1 - sp.getCcov(flgdiag))
                  * CoVar[i][j]
                  + sp.getCcov()
                  * (1. / sp.getMucov())
                  * (pc[i] * pc[j] + (1 - hsig) * sp.getCc()
                  * (2. - sp.getCc()) * CoVar[i][j]);
          for (k = 0; k < sp.getMu(); ++k) { /*
             * additional rank mu
             * update
             */
            CoVar[i][j] += sp.getCcov() * (1 - 1. / sp.getMucov())
                    * sp.getWeights()[k]
  ...
  /** assigns lhs to a differenct instance with the same values,
...
//    }
 ...
 and smallest eigenvalue of covariance matrix CoVar
 <li> 0.1 time, overall elapsed time in seconds
  ...
  public String getPrintLine() {
    ...
    new Object[]{
    new Integer(math.minidx(math.diag(CoVar))),
    new Double(sigma *
    Math.sqrt(math.min(math.diag(CoVar)))),
    new Integer(math.maxidx(math.diag(CoVar))),
    new Double(sigma *
    ...
    if (state < 0) {
      ...
      s = new String(
              ...
              + new PrintfFormat(Locale.US, "%2d:").sprintf(math.maxidx(math.diag(CoVar)))
              + new PrintfFormat(Locale.US, "%8.1e ").sprintf(sigma * maxsqrtdiagC)
              + new PrintfFormat(Locale.US, "%2d:").sprintf(math.minidx(math.diag(CoVar)))
  ...
  public String getDataRowFitness() {
    ...
    if (mode == SINGLE_MODE) {
      ...
      s += math.min(diagD) + " "
              + (math.maxidx(math.diag(CoVar)) + 1) + " " + sigma * maxsqrtdiagC + " "
              + (math.minidx(math.diag(CoVar)) + 1) + " " + sigma * minsqrtdiagC;
      //for (int i = 0; i < sp.getLambda(); ++i) {
      //    s += fit.funValues[i].d + " ";
      //}
    }
  ...
  public String getDataRowStddev() {
    ...
    s = countiter + " " + counteval + " " + sigma + " "
            + (1 + math.maxidx(math.diag(CoVar))) + " " + (1 + math.minidx(math.diag(CoVar))) + " ";
    for (int i = 0; i < N; ++i) {

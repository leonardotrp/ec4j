 ...
 */
/**
 * implements the Covariance Matrix Adaptation Evolution Strategy (CMA-ES) for
 * non-linear, non-convex, non-smooth, global function minimization. The
 * CMA-Evolution Strategy (CMA-ES) is a reliable stochastic optimization method
 * which should be applied, if derivative based methods, e.g. quasi-Newton BFGS
 * or conjugate gradient, fail due to a rugged search landscape (e.g. noise,
 * local optima, outlier, etc.) of the objective function. Like a quasi-Newton
 * method the CMA-ES learns and applies a variable metric of the underlying
 * search space. Unlike a quasi-Newton method the CMA-ES does neither estimate
 * nor use gradients, making it considerably more reliable in terms of finding a
 * good, or even close to optimal, solution, finally.
 *
 * <p>
 ...
 * provided). For up to <math>N=10</math> variables also the derivative-free
 * simplex direct search method (Nelder & Mead) can be faster, but it is far
 * less reliable than CMA-ES.
 *
 * <p>
 ...
 * conventional evolution strategy, it will usually take about
 * 30&#215;<math>N</math> function evaluations. On difficult problems the
 * complete optimization (a single run) is expected to take <em>roughly</em>
 * between <math>30&#215;N</math> and <math>300&#215;N<sup>2</sup></math>
 * function evaluations.
 * 
 * <p>
 * The main functionality is provided by the methods
 * <code>double[][] {@link #samplePopulation()}</code> and
 * <code>{@link #updateDistribution(double[])}</code> or
 * <code>{@link #updateDistribution(double[][], double[])}</code>. Here is an
 * example code snippet, see file <tt>CMAExample1.java</tt> for a similar
 * example, and <tt>CMAExample2.java</tt> for a more extended example with
 * multi-starts implemented.
 * 
 * <pre>
 ...
 * </P>
 * <P>
 * The implementation follows very closely <a name=HK2004>[3]</a>. It supports
 * small and large population sizes, the latter by using the rank-&micro;-update
 * [2], together with weighted recombination for the covariance matrix, an
 * improved parameter setting for large populations [3] and an (initially)
 * diagonal covariance matrix [5]. The latter is particularly useful for large
 * dimension, e.g. larger 100. The default population size is small [1]. An
 * independent restart procedure with increasing population size [4] is
 * implemented in class <code>{@link cmaes.examples.CMAExample2}</code>.
 * </P>
 * 
 * <P>
 * <B>Practical hint</B>: In order to solve an optimization problem in
 * reasonable time it needs to be reasonably encoded. In particular the domain
 * width of variables should be similar for all objective variables (decision
 * variables), such that the initial standard deviation can be chosen the same
 * for each variable. For example, an affine-linear transformation could be
 * applied to each variable, such that its typical domain becomes the interval
 * [0,10]. For positive variables a log-encoding or a square-encoding should be
 * considered, to avoid the need to set a hard boundary at zero, see
 * <A href="http://www.lri.fr/~hansen/cmaes_inmatlab.html#practical">here for a
 * few more details</A>.
 * </P>
 * 
 * <P>
 * <B>References</B>
 * <UL>
 * <LI>[1] Hansen, N. and A. Ostermeier (2001). Completely Derandomized
 * Self-Adaptation in Evolution Strategies. <I>Evolutionary Computation</I>,
 * 9(2), pp. 159-195.</LI>
 * <LI>[2] Hansen, N., S.D. M&uuml;ller and P. Koumoutsakos (2003). Reducing the
 * Time Complexity of the Derandomized Evolution Strategy with Covariance Matrix
 * Adaptation (CMA-ES). <I>Evolutionary Computation</I>, 11(1), pp. 1-18.
 * 
 * <LI>[3] Hansen and Kern (2004). Evaluating the CMA Evolution Strategy on
 * Multimodal Test Functions. In <I> Eighth International Conference on Parallel
 * Problem Solving from Nature PPSN VIII, Proceedings</I>, pp. 282-291, Berlin:
 * Springer.</LI>
 * <LI>[4] Auger, A, and Hansen, N. (2005). A Restart CMA Evolution Strategy
 * With Increasing Population Size.</A> In <I>Proceedings of the IEEE Congress
 * on Evolutionary Computation, CEC 2005</I>, pp.1769-1776.</LI>
 * <LI>[5] Ros, R. and N. Hansen (2008). A Simple Modification in CMA-ES
 * Achieving Linear Time and Space Complexity. In Rudolph et al. (eds.)
 * <I>Parallel Problem Solving from Nature, PPSN X, Proceedings</I>, pp.
 * 296-305, Springer.</LI>
 * </UL>
 * </P>
 * 
...
public class CMAEvolutionStrategy implements java.io.Serializable {
	...
	public class StopCondition {
		...
		/**
		 * true whenever a termination criterion was met. clear() re-sets this value to
		 * false.
		 * 
		 * @see #clear()
		 */
		...
		/**
		 * greater than zero whenever a termination criterion was satisfied, zero
		 * otherwise. clear() re-sets this value to zero.
		 * 
		...
		/**
		 * get description messages of satisfied termination criteria. The messages
		 * start with one of "Fitness:", "TolFun:", "TolFunHist:", "TolX:", "TolUpX:",
		 * "MaxFunEvals:", "MaxIter:", "ConditionNumber:", "NoEffectAxis:",
		 * "NoEffectCoordinate:".
		 * 
		 * @return String[] s with messages of termination conditions. s[0].equals("")
		 *         is true if no termination condition is satisfied yet
		 */
		 ...
		 * termination criterion is satisfied. Repeated tests append the met criteria
		 * repeatedly, only if the evaluation count has changed.
		 * 
		...
		int test() {
			...
			if (index > 0 && (counteval == lastcounteval || counteval == lastcounteval + 1)) // one evaluation for xmean is ignored
			{
			...
			if ((countiter > 1 || state >= 3) && bestever_fit <= options.stopFitness) {
				appendMessage("Fitness: Objective function value dropped below the target function value " + options.stopFitness);
			}
			...
			/* TOLFUN */
			if ((countiter > 1 || state >= 3) && Math.max(math.max(fit.history), fit.fitness[fit.fitness.length - 1].val) - Math.min(math.min(fit.history), fit.fitness[0].val) <= options.stopTolFun) {
			...
			if (sigma * maxsqrtdiagC > options.stopTolUpXFactor * maxstartsigma) {
				appendMessage("TolUpX: standard deviation increased by more than stopTolUpXFactor=" + options.stopTolUpXFactor + ", larger initial standard deviation recommended");
			}
			...
			/*
			 * Test each principal axis i, whether x == x + 0.1 * sigma * rgD[i] * Bmat[i]
			 */
			for (int iAchse = 0; iAchse < N; ++iAchse) {
				...
				for (iKoo = l; iKoo < u; ++iKoo) {
					if (xmean[iKoo] != xmean[iKoo] + fac * Bmat[iKoo][iAchse]) {
						break; // is OK for this iAchse
					}
				}
				if (iKoo == u) // no break, therefore no change for axis iAchse
				{
					appendMessage("NoEffectAxis: Mutation " + 0.1 * sigma * diagD[iAchse] + " in a principal axis " + iAchse + " has no effect");
				}
			} /* for iAchse */

			/* Test whether one component of xmean is stuck */
			for (int iKoo = 0; iKoo < N; ++iKoo) {
				if (xmean[iKoo] == xmean[iKoo] + 0.2 * sigma * Math.sqrt(CoVar[iKoo][iKoo])) {
					appendMessage("NoEffectCoordinate: Mutation of size " + 0.2 * sigma * Math.sqrt(CoVar[iKoo][iKoo]) + " in coordinate " + iKoo + " has no effect");
				}
			} /* for iKoo */
	...
	void testAndCorrectNumerics() { // not much left here
		...
		if (getCountIter() > 1 || (getCountIter() == 1 && state >= 3)) {
			if (fit.fitness[0].val == fit.fitness[Math.min(sp.getLambda() - 1, sp.getLambda() / 2 + 1) - 1].val) {
				double fac = Math.exp(0.2 + sp.getCs() / sp.getDamps());
				sigma *= fac;
				// warning("flat fitness landscape, consider reformulation of fitness, step-size
				// increased by factor " + fac);
			}
		}

		/* Align (renormalize) scale CoVar (and consequently sigma) */
		/*
		 * e.g. for infinite stationary state simulations (noise handling needs to be
		 * introduced for that)
		 */
		double fac = 1;
	...
	long counteval;
	long countiter;
	long bestever_eval; // CoVar style, TODO: better make use of class CMASolution?
	double[] bestever_x;
	...
	/**
	 * postpones most initialization. For initialization use setInitial... methods
	 * or set up a properties file, see file "CMAEvolutionStrategy.properties".
	 */
	...
	/**
	 * retrieves options and strategy parameters from properties input, see file
	 * <tt>CMAEvolutionStrategy.properties</tt> for valid properties
	 */
	...
	/**
	 * reads properties (options, strategy parameter settings) from file
	 * <code>propertiesFileName</code>
	 */
	...
	/**
	 * initialization providing all mandatory input arguments at once. The following
	 * two is equivalent
	 * 
	 * <PRE>
	 * cma.init(N, X, SD);
	 ...
	 * </PRE>
	 *
	 * The call to <code>init</code> is a point of no return for parameter settings,
	 * and demands all mandatory input be set. <code>init</code> then forces the
	 * setting up of everything and calls
	 * <code>parameters.supplementRemainders()</code>. If <code>init</code> was not
	 * called before, it is called once in <code>samplePopulation()</code>. The
	 * return value is only provided for sake of convenience.
	 *
	 * @param dimension
	 * @param initialX
	 ...
	 *            can be of size one, where all standard deviations are set to the
	 *            same value, or of size dimension
	 *
	 ...
	 *
	 * @param x
	 *            null or x.length==1 or x.length==dim, only for the second case x
	 *            is expanded
	 * @param dim
	...
	public double[] init() {
		...
		if (startsigma != null) { //
			...
			// we might use boundaries here to find startsigma, but I prefer to have stddevs
			// mandatory
		...
		if (xmean == null) {
			...
			if (typicalX != null) {
				...
				error("no initial search point (solution) X or typical X specified"); // TODO Dirk: remove this error - set initial start point to mean value to avoid
																						// starting in a corner of the search space.
				xmean = new double[N];
		...
		ps = new double[N];
		Bmat = new double[N][N];
		CoVar = new double[N][N]; // essentially only i <= j part is used
		...
		for (i = 0; i < N; ++i) {
			...
			Bmat[i][i] = 1;
			CoVar[i][i] = diagD[i] * diagD[i];
		}
		maxsqrtdiagC = Math.sqrt(math.max(math.diag(CoVar)));
		minsqrtdiagC = Math.sqrt(math.min(math.diag(CoVar)));
		...
		countiter = 0; // added, Dirk, 13.3.2013
		counteval = 0; // added, Dirk, 13.3.2013
		countwritten = 0; // added, Dirk, 13.3.2013
	...
	/**
	 * reads properties from default input file CMAEvolutionStrategy.properties and
	 * sets options and strategy parameter settings accordingly. Options values can
	 * be changed at any time using this function.
	 */
	...
	public Properties readProperties(String fileName) {
		InputStream propertiesFile = this.getClass().getResourceAsStream("/" + this.getClass().getPackage().getName().replace('.', '/') + '/' + fileName);
	...
	/**
	 * reads properties from Properties class input and sets options and parameters
	 * accordingly
	 *
	 * @param properties
	...
	private void error(String s) { // somehow a relict from the CoVar history of this code
	...
	class MyMath { // implements java.io.Serializable {
		...
		/**
		 * @return index of minium value of 1-D double array between index 0 and maxidx
		 * @param ar
		 *            double[]
		 * @param maxidx
	...
	void eigendecomposition(int flgforce) {
		/* Update Bmat and D, calculate eigendecomposition */
		int i, j;
		...
		// 20% is usually better in terms of running *time* (only on fast to evaluate
		// functions)
		if (!flgdiag && flgforce <= 0 && (timings.eigendecomposition > 1000 + options.maxTimeFractionForEigendecomposition * (System.currentTimeMillis() - timings.starteigen) || countCupdatesSinceEigenupdate < 1. / sp.getCcov() / N / 5.)) {
			return;
		}

		if (flgdiag) {
			for (i = 0; i < N; ++i) {
				diagD[i] = Math.sqrt(CoVar[i][i]);
			}
			countCupdatesSinceEigenupdate = 0;
			...
			tred2(N, Bmat, diagD, offdiag);
			tql2(N, diagD, offdiag, Bmat);
			timings.eigendecomposition += System.currentTimeMillis() - firsttime;

			if (options.checkEigenSystem > 0) {
				checkEigenSystem(N, CoVar, diagD, Bmat); // for debugging
			}
	...
	int checkEigenSystem(int N, double C[][], double diag[], double Q[][]) /*
		...
		for (i = 0; i < N; ++i) {
			for (j = 0; j < N; ++j) {
				...
				if (Math.abs(cc - C[i > j ? i : j][i > j ? j : i]) / Math.sqrt(C[i][i] * C[j][j]) > 1e-10 && Math.abs(cc - C[i > j ? i : j][i > j ? j : i]) > 1e-9) { /* quite large */
	...
	public double[][] samplePopulation() {
		...
		if (state < 0) {
			...
			eigendecomposition(0); // latest possibility to generate Bmat and diagD
		}
		if (state != 1) {
			++countiter;
		}
		...
		for (iNk = 0; iNk < sp.getLambda(); ++iNk) { /*
			...
			// code duplication from resampleSingle because of possible future resampling
			// before GenoPheno
			/* generate scaled random vector (D * z) */
			if (flgdiag) {
				for (i = 0; i < N; ++i) {
				...
				/* add mutation (sigma * Bmat * (D*z)) */
				for (i = 0; i < N; ++i) {
					for (j = 0, sum = 0; j < N; ++j) {
						sum += Bmat[i][j] * artmp[j];
					}
	...
	/**
	 * sets the design variables of the population after bound handling.
	 * 
	 * Added D. Bueche for opendino, 30.3.2016.
	 * 
	 * @param x
	 *            design variables of the population.
	 */
	public void setPopulation(double[][] x) {
		phenoGenoTransformation(x, arx);
	}
	...
	/**
	 * re-generate the <code>index</code>-th solution. After getting lambda solution
	 * points with samplePopulation() the i-th point, i=0...lambda-1, can be sampled
	 * anew by resampleSingle(i).
	 *
	 * <PRE>
	...
	/**
	 * compute Mahalanobis norm of x - mean w.r.t. the current distribution (using
	 * covariance matrix times squared step-size for the inner product). TODO: to be
	 * tested.
	 * 
	 * @param x
	 * @param mean
	 * @return Malanobis norm of x - mean: sqrt((x-mean)' CoVar^-1 (x-mean)) / sigma
	 */
	public double mahalanobisNorm(double[] x, double[] mean) {
		double yi, snorm = 0;
		int i, j;
		// snorm = (x-mean)' Cinverse (x-mean) = (x-mean)' (BD^2B')^-1 (x-mean)
		// = (x-mean)' Bmat'^-1 D^-2 Bmat^-1 (x-mean)
		// = (x-mean)' Bmat D^-1 D^-1 Bmat' (x-mean)
		// = (D^-1 Bmat' (x-mean))' * (D^-1 Bmat' (x-mean))
		/*
		 * calculate z := D^(-1) * Bmat^(-1) * BDz into artmp, we could have stored z
		 * instead
		 */
		for (i = 0; i < N; ++i) {
			for (j = 0, yi = 0.; j < N; ++j) {
				yi += Bmat[j][i] * (x[j] - mean[j]);
			}
			// yi = i-th component of Bmat' (x-mean)
	...
	/**
	 * update of the search distribution from a population and its function values,
	 * see {@link #updateDistribution(double[][], double[], 0)}. This might become
	 * updateDistribution(double[][], double[], popsize) in future.
	 *
	 * @param population
	 ...
	 * {@link #updateDistribution(double[] functionValues)}. functionValues is used
	 * to establish an ordering of the elements in population. The first nInjected
	 * elements do not need to originate from #samplePopulation() or can have been
	 * modified (TODO: to be tested).
	 *
	 * @param population
	 *            double[lambda][N], lambda solutions
	 * @param functionValues
	 ...
	 * determines the selection order (ranking) for the solutions in the previously
	 * sampled population. This is just a different interface for
	...
	public void updateDistribution(double[] functionValues) {
		...
		if (functionValues.length != sp.getLambda()) {
			error("argument double[] funcionValues.length=" + functionValues.length + "!=" + "lambda=" + sp.getLambda());
		}
	...
	// // TODO: figure out whether the change of penalty is too large or fast which
	// can disturb selection
	...
	// medianDeltaFit = Math.min(medianDeltaFit, Math.abs(fit.funValues[ii+i].d -
	// fit.funValues[i].d));
	// medianDeltaFit /= sigma * sigma; // should be somehow constant, because dfit
	// depends on sigma (verified on sphere)
	// if (medianDeltaFit > 0) {
	...
	// /* calculate CoVar^-1-norm of Delta x: norm(D^(-1) * Bmat^(-1) * (Delta x))^2
	// */
	// for (i = 0; i < N; ++i) {
	// double sum = 0.0;
	// for (j = 0, sum = 0.; j < N; ++j)
	// sum += Bmat[j][i] * ((arxrepaired[fit.funValues[iNk].i][j] -
	// arx[fit.funValues[iNk].i][j]));
	// sqrnorm += math.square(sum / (Math.pow(diagD[i], 0.9) * Math.pow(prod,
	// 0.10))); // regularization to I
	// }
	...
	private void updateDistribution() {
		...
		/* calculate xmean and BDz~N(0,CoVar) */
		for (i = 0; i < N; ++i) {
			xold[i] = xmean[i];
			xmean[i] = 0.;
		...
		/* cumulation for sigma (ps) using Bmat*z */
		if (flgdiag) {
			/* given Bmat=I we have Bmat*z = z = D^-1 BDz */
			for (i = 0; i < N; ++i) {
		...
		/* cumulation for covariance matrix (pc) using Bmat*D*z~N(0,CoVar) */
		hsig = 0;
		...
		/*
		 * this, it is harmful in a dynamic environment remove momentum in ps, if ps is
		 * large and fitness is getting worse
		 */
		...
		if (sp.getCcov() > 0 && iniphase == false) {
			...
			for (i = 0; i < N; ++i) {
				for (j = (flgdiag ? i : 0); j <= i; ++j) {
					CoVar[i][j] = (1 - sp.getCcov(flgdiag)) * CoVar[i][j] + sp.getCcov() * (1. / sp.getMucov()) * (pc[i] * pc[j] + (1 - hsig) * sp.getCc() * (2. - sp.getCc()) * CoVar[i][j]);
					for (k = 0; k < sp.getMu(); ++k) { /*
														 * additional rank mu update
														 */
						CoVar[i][j] += sp.getCcov() * (1 - 1. / sp.getMucov()) * sp.getWeights()[k] * (arx[fit.fitness[k].i][i] - xold[i]) * (arx[fit.fitness[k].i][j] - xold[j]) / sigma / sigma;
					}
				}
			}
			maxsqrtdiagC = Math.sqrt(math.max(math.diag(CoVar)));
		...
		// ensure minimal standard deviations // DIRK
		if (options.lowerStandardDeviations != null) {
			if (options.lowerStandardDeviations.length != N) {
				throw new RuntimeException("number of lower standard deviations must agree with number of design variables");
			}
			sigma = CMAMinStep.setMinStep(diagD, Bmat, CoVar, sigma, options.lowerStandardDeviations);
		}

	} // updateDistribution()

	/**
	 * assigns lhs to a differenct instance with the same values, sort of smart
	 ...
	 * ellipsoid, which is the square root of the largest divided by the smallest
	...
	/**
	 * get best evaluated solution found so far. Remark that the distribution mean
	 * was not evaluated but is expected to have an even better function value.
	 * <p>
	 ...
	 *
	 * @param fitness
	 *            function value computed for the solution {@link #getMeanX()}
	...
	/**
	 * get best evaluated search point found so far. Remark that the distribution
	 * mean was not evaluated but is expected to have an even better function value.
	 * 
	...
	/**
	 * Get best evaluated solution of the last (recent) iteration. This solution is
	 * supposed to be more robust than the best ever solution in particular in
	 * possible case of mis-attributed good fitness values. Remark that the
	 * distribution mean was not evaluated but is expected to have an better
	 * function value.
	 * 
	...
	/**
	 * Get mean of the current search distribution. The mean should be regarded as
	 * the best estimator for the global optimimum at the given iteration. In
	 * particular for noisy problems the distribution mean is the solution of choice
	 * preferable to the best or recent best. The return value is <em>not</em> a
	 * copy. Therefore it should not be change it, without deep knowledge of the
	 * code (the effect of a mean change depends on the chosen
	 ...
	 *
	 * @return <code>double[] initialX</code> start point chosen for distribution
	 *         mean value <code>xmean</code>
	 */
	...
	// *
	// * @param xlow lower boundary double[], can be 1-D or of length of the number
	...
	public void setInitialStandardDeviations(double[] startsigma) {
		...
		if (N > 0 && N != startsigma.length) {
			error("dimensions N=" + N + " and input startsigma.length=" + startsigma.length + "do not agree");
		}
		if (N == 0) {
	...
	/**
	 * set initial seach point <code>xmean</code> coordinate-wise uniform between
	 * <code>l</code> and <code>u</code>, dimension needs to have been set before
	 *
	 * @param l
	 *            double lower value
	 * @param u
	 ...
	 * <code>x.length==1</code> is possible, otherwise the search space dimension is
	 * set to <code>x.length</code> irrevocably
	 *
	 * @param x
	 ...
	 * Changing the seed will only take effect before {@link #init()} was called.
	 * 
	 * @param seed
	 ...
	 * </pre>
	 * 
	 * shows the value of getPrintAnnotation() in the first line and in the second
	 * line
	 * <li>164 iteration number
	 * <li>( 8) recently sampled search point in this iteration,
	 * <li>1638: number of function evaluations
	 * <li>5.5061568003892640e-08 objective function value F of the best point in
	 ...
	 * <li>index of coordinate with smallest standard deviation: respective standard
	 * deviation
	 * <li>| 1.4e-05 standard deviation in smallest principal axis direction
	 * <li>5.6e-05 sigma
	 * <li>2.34 axisratio, ie. quotient between the standard deviations in largest
	 * an smallest principal axis directions, ie. square root of the quotient
	 * between largest and smallest eigenvalue of covariance matrix CoVar
	...
	public String getPrintLine() {
		 ...
		 * String.format(Locale.US, "%1$.16e (%2$+.0e %3$.0e)", new Object[]{ new
		 * Double(recentFunctionValue), new Double(getBestFunctionValue() -
		 * recentFunctionValue), new Double(recentMaxFunctionValue -
		 * recentFunctionValue) }) + String.format(Locale.US, "%1$7.2f ", new Object[]{
		 * new Double(axisratio) }) + String.format(Locale.US,
		 * "%1$2d:%2$8.1e %3$2d:%4$8.1e", new Object[]{ new
		 * Integer(math.minidx(math.diag(CoVar))), new Double(sigma *
		 * Math.sqrt(math.min(math.diag(CoVar)))), new
		 * Integer(math.maxidx(math.diag(CoVar))), new Double(sigma *
		 * Math.sqrt(math.max(math.diag(CoVar)))) })
		 */
		String s;
		if (state < 0) {
			...
			s = new String(new PrintfFormat(Locale.US, " %4d").sprintf(countiter) + new PrintfFormat(Locale.US, "(%2d), ").sprintf(idxRecentOffspring + 1) + new PrintfFormat(Locale.US, "%6.0d: ").sprintf(counteval)
					+ new PrintfFormat(Locale.US, "%.16e ").sprintf(recentFunctionValue) + new PrintfFormat(Locale.US, "(%+.0e,").sprintf(getBestFunctionValue() - recentFunctionValue)
					+ new PrintfFormat(Locale.US, "%.0e) | ").sprintf(recentMaxFunctionValue - recentFunctionValue) + new PrintfFormat(Locale.US, "%2d:").sprintf(math.maxidx(math.diag(CoVar)))
					+ new PrintfFormat(Locale.US, "%8.1e ").sprintf(sigma * maxsqrtdiagC) + new PrintfFormat(Locale.US, "%2d:").sprintf(math.minidx(math.diag(CoVar))) + new PrintfFormat(Locale.US, "%8.1e ").sprintf(sigma * minsqrtdiagC)
					+ new PrintfFormat(Locale.US, "| %6.1e ").sprintf(sigma * math.min(diagD)) + new PrintfFormat(Locale.US, "%6.1e ").sprintf(sigma) + new PrintfFormat(Locale.US, "%6.2f").sprintf(axisratio)
		 ...
		 */
		/*
		 * formatting template String.format(Locale.US, "%1$6.2e %2$+.0e", new Object[]{
	...
	public String getPrintAnnotation() {
		...
		// 491( 3), 3924: 1.1245467061992267e+00 (-2e-01,4e-01) 9: 7.8e-05 2: 5.0e-02 |
		// 5.9e-03 1.3e-02 660.41
	...
	public String helloWorld() {
		String s = new String("(" + sp.getMu() + "," + sp.getLambda() + ")-CMA-ES(mu_eff=" + Math.round(10. * sp.getMueff()) / 10. + "), Ver=\"" + versionNumber + "\", dimension=" + N + ", " + options.diagonalCovarianceMatrix
				+ " diagonal iter." + ", randomSeed=" + seed + " (" + new Date().toString() + ")");
		return s;

	}

	/**
	 * calls System.out.println(s) and writes s to the file outcmaesdisp.dat by
	 * default, if writeDisplayToFile option is > 0
	 * 
	 * @see #getPrintLine()
	 */
	public void println(String s) {
		if (options.writeDisplayToConsole > 0)
	...
	public String getDataRowFitness() {
		...
		if (mode == SINGLE_MODE) {
			...
			s += math.min(diagD) + " " + (math.maxidx(math.diag(CoVar)) + 1) + " " + sigma * maxsqrtdiagC + " " + (math.minidx(math.diag(CoVar)) + 1) + " " + sigma * minsqrtdiagC;
			// for (int i = 0; i < sp.getLambda(); ++i) {
			// s += fit.funValues[i].d + " ";
			// }
		}
	...
	public String getDataRowStddev() {
		String s = new String();
		s = countiter + " " + counteval + " " + sigma + " " + (1 + math.maxidx(math.diag(CoVar))) + " " + (1 + math.minidx(math.diag(CoVar))) + " ";
		for (int i = 0; i < N; ++i) {
			s += sigma * Math.sqrt(CoVar[i][i]) + " ";
		}
		return s;
	}
	 ...
	 * '%#'-commented row contains itertation number, evaluation number, and sigma.
	 * In the remaining rows the upper triangular part contains variances and
	 * covariances sigma*sigma*c_ij. The lower part contains correlations c_ij /
	 * sqrt(c_ii * c_jj).
	 */
	public String getDataC() {
		...
		for (i = 0; i < N; ++i) {
			...
			{
				s += CoVar[i][j] / Math.sqrt(CoVar[i][i] * CoVar[j][j]) + " ";
			}
			for (j = i; j < N; ++j) {
				s += sigma * sigma * CoVar[i][j] + " ";
			}
			s += "\n";
		}
		return s;
	 ...
	 * @param flgAppend
	 *            for flgAppend>0 old data are not overwritten
	 */
	...
	/**
	 * writes data output to default files. Uses opts.outputFileNamesPrefix to
	 * create filenames. Columns 1-2 are iteration number and function evaluation
	 * count, columns 6- are the data according to the filename. Maximum time spent
	 * for writing can be controlled in the properties file.
	 *
	 * <p>
	 * The output is written to files that can be printed in Matlab or Scilab (a
	 * free and easy to install Matlab "clone").
	 * </p>
	 * <p>
	 * Matlab:
	 * 
	...
	/**
	 * writes data output to default files. Maximum time spent for writing can be
	 * controlled in the properties file. For negative values no writing takes
	 * place, overruling the <code>flgForce</code> input parameter below.
	 *
	 * @param flgForce
	 *            0==write depending on time spent with writing, 1==write if the
	 *            iteration count has changed, 2==write always, overruled by
	 *            negative values of maxTimeFractionForWriteToDefaultFiles property
	 *
	...
	public void writeToDefaultFiles(String fileNamePrefix) {
		...
		if (citerlastwritten >= 0) { // negative value forces writing
			...
			// iteration gap is less than two times of the average gap, to not have large
			// data holes
		...
		writeToFile(fileNamePrefix + "stddev.dat", getDataRowStddev(), 1); // sigma*sqrt(diag(CoVar))
		...
		// opts.maxTimeFractionForWriteToDefaultFiles);
		if (countiter < 3) {
	 ...
	 *
	 * @param flgAppend
	 *            == 0 means overwrite files, == 1 means append to files
	 */
	 ...
	 * ...xbest.dat, ...std.dat, ...axlen.dat, and in case the first data line,
	 * usually with the initial values.
	 * 
	 * @param fileNamePrefix
	...
	/**
	 * very provisional error handling. Methods of the class CMAEvolutionStrategy
	 * might throw the CMAException, that need not be catched, because it extends
	 * the "unchecked" RuntimeException class
	 */
